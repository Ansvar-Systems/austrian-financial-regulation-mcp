/**
 * SQLite database access layer for the FMA MCP server.
 *
 * Schema mirrors the FMA regulatory document structure:
 *   - sourcebooks        — top-level FMA document categories (Rundschreiben, Mindeststandards, etc.)
 *   - provisions         — individual requirements, guidance, and standards
 *   - enforcement_actions — FMA enforcement decisions and supervisory actions
 *
 * FTS5 virtual tables back full-text search on provisions and enforcement actions.
 * Primary language: German (queries and content expected in German).
 */
...
export function searchProvisions(opts: SearchProvisionsOptions): Provision[] {
  ...
  if (opts.sourcebook ?? opts.status) {
    ...
    return db
      .prepare(
        "SELECT p.* FROM provisions_fts f JOIN provisions p ON p.id = f.rowid WHERE provisions_fts MATCH :query " + where + " ORDER BY rank LIMIT :limit",
      )
  ...
  return db
    .prepare(
      "SELECT p.* FROM provisions_fts f JOIN provisions p ON p.id = f.rowid WHERE provisions_fts MATCH :query ORDER BY rank LIMIT :limit",
    )
...
export function getProvision(
  ...
  // Fallback: partial match on reference
  return (
    (db
      .prepare(
...
export function searchEnforcement(
  ...
  if (opts.action_type) {
    return db
      .prepare(
        "SELECT e.* FROM enforcement_fts f JOIN enforcement_actions e ON e.id = f.rowid WHERE enforcement_fts MATCH :query AND e.action_type = :action_type ORDER BY rank LIMIT :limit",
      )
  ...
  return db
    .prepare(
      "SELECT e.* FROM enforcement_fts f JOIN enforcement_actions e ON e.id = f.rowid WHERE enforcement_fts MATCH :query ORDER BY rank LIMIT :limit",
    )
